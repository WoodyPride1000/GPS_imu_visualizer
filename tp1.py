...
def home():
    """
    アプリケーションのルートURLにアクセスしたときに表示されるシンプルなウェルカムページ。
    APIの使用方法に関する基本的な情報を提供します。
    """
    return """
    <!DOCTYPE html>
    <html lang="ja">
    <head>
        ...
        <style>
            ...
            .note { background-color: #fff3cd; border-left: 5px solid #ffe08a; padding: 10px; margin-top: 20px; border-radius: 4px; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>ようこそ！センサーデータAPIへ</h1>
            <p>このアプリケーションは、GPSとIMUセンサーデータを統合して、車両やシステムのヘディング（方位）や位置情報を提供します。</p>

            <h2>利用可能なAPIエンドポイント</h2>
            <ul>
                <li><strong>GET /api/data</strong>: 現在のセンサーおよび計算データをJSON形式で取得します。</li>
                <li><strong>POST /api/calibration/start</strong>: IMUのジャイロZ軸オフセットキャリブレーションを開始します。</li>
                <li><strong>POST /api/calibration/stop</strong>: IMUのジャイロZ軸オフセットキャリブレーションを終了し、オフセット値を計算・適用します。</li>
                <li><strong>POST /api/log_level</strong>: アプリケーションのログレベルを変更します。</li>
            </ul>

            <h2>APIキーについて</h2>
            <div class="note">
                <strong>重要:</strong> <code>/api/data</code>, <code>/api/calibration/*</code>, <code>/api/log_level</code> エンドポイントには <strong>APIキー認証</strong> が必要です。<br>
                リクエストヘッダに <code>X-API-Key: YOUR_SECURE_API_KEY_HERE</code> を含めてください。<br>
                APIキーは <code>config.ini</code> ファイルで設定されています。
            </div>

            <h2>使用例 (<code>curl</code> コマンド)</h2>
            <p><code>YOUR_API_KEY_HERE</code> は、<code>config.ini</code> に設定した実際のAPIキーに置き換えてください。</p>

            <h3>現在のセンサーデータを取得 (GET)</h3>
            <pre><code>curl -H "X-API-Key: YOUR_API_KEY_HERE" http://127.0.0.1:5000/api/data</code></pre>

            <h3>IMUキャリブレーションを開始 (POST)</h3>
            <pre><code>curl -X POST -H "X-API-Key: YOUR_API_KEY_HERE" http://127.0.0.1:5000/api/calibration/start</code></pre>

            <h3>ログレベルをDEBUGに変更 (POST)</h3>
            <pre><code>curl -X POST -H "X-API-Key: YOUR_API_KEY_HERE" -H "Content-Type: application/json" -d '{"level": "DEBUG"}' http://127.0.0.1:5000/api/log_level</code></pre>

            <h2>ダミーモードについて</h2>
            <p><code>config.ini</code> で <code>DummyMode = True</code> に設定すると、センサーなしでデバッグや開発が可能です。このモードではAPIキー認証はスキップされ、HTTPで動作します。</p>
...
def get_current_data():
    ...
    with sensor_data.lock:
        data = {
            "base_lat": round(sensor_data.base_data['lat'], 7),
            "base_lon": round(sensor_data.base_data['lon'], 7),
            "base_hdop": round(sensor_data.base_data['hdop'], 2),
            ...
            "rover_hdop": round(sensor_data.rover_data['hdop'], 2),
            ...
            "error_m": round(sensor_data.error, 3),
            "distance_m": round(sensor_data.distance, 3),
            "imu_status": sensor_data.imu_status,
            "imu_accel_x": round(sensor_data.imu_accel_x, 5),
            "imu_accel_y": round(sensor_data.imu_accel_y, 5),
            "imu_accel_z": round(sensor_data.imu_accel_z, 5),
            "imu_gyro_x": round(sensor_data.imu_gyro_x, 5),
            "imu_gyro_y": round(sensor_data.imu_gyro_y, 5),
            "imu_raw_gyro_z": round(sensor_data.imu_raw_gyro_z, 5),
...
def set_log_level():
    ...
    try:
        ...
        for handler in logging.root.handlers:
            if isinstance(handler, logging.StreamHandler) or isinstance(handler, logging.handlers.RotatingFileHandler):
                handler.setLevel(level)
        logger.info(f"ログレベルを {logging.getLevelName(level)} に変更しました。要求元: {request.remote_addr}")
        return jsonify({"status": "ok", "message": f"ログレベルが {logging.getLevelName(level)} に変更されました。", "current_level": logging.getLevelName(level)})
    except AttributeError:
        logger.warning(f"無効なログレベルが指定されました: {level_str}。要求元: {request.remote_addr}")
        return jsonify({"status": "error", "message": f"無効なログレベル: {level_str}。有効なレベルは DEBUG, INFO, WARNING, ERROR, CRITICAL です。"}), 400

# --- サーバー起動とスレッド管理 ---
def run_app():
    # Flaskサーバーのホストとポートを固定 (またはconfig.iniから読み込む)
    flask_host = '0.0.0.0'
    flask_port = 5000
    ...
    if not DUMMY_MODE:
        try:
            ...
            app.run(host=flask_host, port=flask_port, ssl_context=context, debug=False)
